...
next?.addEventListener('click',()=>{if(idx<scenes.length-1){idx++;render()}else{root.classList.add('hidden');setTimeout(()=>root.remove(),380)}});
dots.forEach((d,i)=>d.addEventListener('click',()=>{idx=i;render()}));
enter?.addEventListener('click',()=>{root.classList.add('hidden');setTimeout(()=>root.remove(),380)});
